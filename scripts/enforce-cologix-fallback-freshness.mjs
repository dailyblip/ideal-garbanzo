...
function runSelfTest() {
  ...
  if (fresh.expired) throw new Error('Cologix fallback expired before 96 hours');
  const boundary = freshnessDecision({ verifiedAt: '2026-09-06T12:00:00Z', nowMs: now, hasPublishedRoles: true });
  if (!boundary.expired) throw new Error('Cologix fallback did not expire at 96 hours');
  const missing = freshnessDecision({ verifiedAt: null, nowMs: now, hasPublishedRoles: true });
  if (!missing.expired) throw new Error('Cologix roles without verification evidence did not fail closed');
  const empty = freshnessDecision({ verifiedAt: null, nowMs: now, hasPublishedRoles: false });
  if (empty.expired) throw new Error('Empty Cologix feed incorrectly entered expiry');
  console.log('Cologix fallback freshness regression tests passed.');
}
...
if (!decision.expired) {
  ...
  if (hasPublishedRoles) console.log(`Cologix verification is ${age} hours old and inside the ${MAX_FALLBACK_AGE_HOURS}-hour maximum.`);
  else console.log('No Cologix roles are currently published; stale-fallback enforcement is not active.');
  process.exit(0);
}
...
status.cologix = {
  ...
  fallbackFreshness: {
    ...
    policy: 'Cologix employer-direct roles may remain published for at most 96 hours after the last successful official Lever verification.'
  }
};
...
console.warn(`Expired Cologix snapshot after ${roundedAgeHours ?? 'unknown'} hours without official-source verification; removed ${removedPublic} public role(s) and ${removedSnapshot} snapshot role(s).`);
