...
function runSelfTest() {
  ...
  if (fresh.expired) throw new Error('DataBank fallback expired before 96 hours');
  const boundary = freshnessDecision({ verifiedAt: '2026-09-06T12:00:00Z', nowMs: now, hasPublishedRoles: true });
  if (!boundary.expired) throw new Error('DataBank fallback did not expire at 96 hours');
  const missing = freshnessDecision({ verifiedAt: null, nowMs: now, hasPublishedRoles: true });
  if (!missing.expired) throw new Error('DataBank roles without verification evidence did not fail closed');
  const empty = freshnessDecision({ verifiedAt: null, nowMs: now, hasPublishedRoles: false });
  if (empty.expired) throw new Error('Empty DataBank feed incorrectly entered expiry');
  console.log('DataBank fallback freshness regression tests passed.');
}
...
const verifiedAt = clean(source.lastHealthyAt || source.checkedAt || source.fallbackFreshness?.lastHealthyAt);
const nowMs = Date.now();
const decision = freshnessDecision({ verifiedAt, nowMs, hasPublishedRoles });

if (!decision.expired) {
  const age = Math.round((decision.ageHours || 0) * 10) / 10;
  if (hasPublishedRoles) console.log(`DataBank verification is ${age} hours old and inside the ${MAX_FALLBACK_AGE_HOURS}-hour maximum.`);
  else console.log('No DataBank roles are currently published; stale-fallback enforcement is not active.');
  process.exit(0);
}
...
status.databank = {
  ...
  fallbackFreshness: {
    ...
    policy: 'DataBank employer-direct roles may remain published for at most 96 hours after the last successful official TalentReef verification.'
  }
};
...
console.warn(`Expired DataBank snapshot after ${roundedAgeHours ?? 'unknown'} hours without official-source verification; removed ${removedPublic} public role(s) and ${removedSnapshot} snapshot role(s).`);
