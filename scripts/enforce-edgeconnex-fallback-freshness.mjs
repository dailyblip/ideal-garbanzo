...
function isEdgeConneX(job = {}) {
  if (clean(job.company) === COMPANY) return true;
  try {
    const url = new URL(clean(job.sourceUrl));
    return url.hostname.toLowerCase() === 'ats.rippling.com' && /^\/edgeconnex\/jobs\//i.test(url.pathname);
  } catch {
    return false;
  }
}
...
function runSelfTest() {
  ...
  if (!fresh.active || fresh.expired) throw new Error('EdgeConneX fallback expired before 96 hours.');
  if (!boundary.expired || boundary.active) throw new Error('EdgeConneX fallback did not expire at the 96-hour boundary.');
  if (!missing.expired) throw new Error('EdgeConneX fallback without verification evidence did not fail closed.');
  if (healthy.expired || healthy.active) throw new Error('Healthy EdgeConneX source incorrectly entered fallback mode.');
  if (empty.expired || empty.active) throw new Error('Empty EdgeConneX snapshot incorrectly entered fallback mode.');

  console.log('EdgeConneX fallback freshness regression tests passed.');
}
...
if (!Array.isArray(snapshot)) throw new Error('EdgeConneX snapshot must contain an array.');

const source = status?.edgeconnexCareers && typeof status.edgeconnexCareers === 'object'
  ? status.edgeconnexCareers
  : {};
const sourceHealthy = source.sourceHealthy === true;
const publicRoles = jobs.filter(isEdgeConneX);
...
if (checkOnly) {
  ...
  if ((snapshot.length || publicRoles.length) && !Number.isFinite(lastHealthyMs)) {
    violations.push('published EdgeConneX roles have no valid lastHealthyAt verification anchor');
  }
  if (sourceHealthy && snapshot.length > 0 && evidenceAgeHours > MAX_HEALTHY_EVIDENCE_AGE_HOURS) {
    violations.push(`healthy EdgeConneX source evidence is ${evidenceAgeHours.toFixed(1)} hours old (limit ${MAX_HEALTHY_EVIDENCE_AGE_HOURS})`);
  }
  if (!sourceHealthy && fallback.expired && (snapshot.length || publicRoles.length)) {
    violations.push(`expired EdgeConneX fallback still retains ${snapshot.length} snapshot / ${publicRoles.length} public role(s)`);
  }
  if (!sourceHealthy && snapshot.length > 0 && !fallback.active) {
    violations.push('unhealthy EdgeConneX source has a retained snapshot outside the verified fallback window');
  }
  if (publicRoles.length !== snapshot.length) {
    violations.push(`EdgeConneX public/snapshot count drift (${publicRoles.length}/${snapshot.length})`);
  }

  if (violations.length) {
    for (const violation of violations) console.error(`EdgeConneX fallback violation: ${violation}`);
    throw new Error(`Blocked ${violations.length} EdgeConneX fallback freshness violation(s).`);
  }

  const mode = sourceHealthy ? 'healthy source evidence' : fallback.active ? 'active verified fallback' : 'empty fail-closed state';
  console.log(`EdgeConneX fallback freshness check passed: ${publicRoles.length} published role(s), ${mode}.`);
  process.exit(0);
}
...
if (!sourceHealthy && fallback.expired && snapshot.length > 0) {
  ...
  nextJobs = jobs.filter(job => !isEdgeConneX(job));
  nextSnapshot = [];
}
...
status.edgeconnexCareers = {
  ...
  publishedRoles: nextJobs.filter(isEdgeConneX).length,
  removedExpiredFallback,
  fallbackPolicy: `Retain the last fully verified EdgeConneX snapshot for at most ${MAX_FALLBACK_AGE_HOURS} hours after official-source failure; then remove EdgeConneX roles until fresh verification succeeds.`
};
...
if (removedExpiredFallback) {
  console.warn(`EdgeConneX fallback expired; removed ${removedExpiredFallback} public role(s) and cleared the stale snapshot.`);
} else if (!sourceHealthy && fallback.active) {
  console.warn(`EdgeConneX source incomplete; retained ${retainedSnapshot} verified role(s) inside the ${MAX_FALLBACK_AGE_HOURS}-hour fallback window.`);
} else {
  console.log(`EdgeConneX source evidence refreshed for ${retainedSnapshot} verified role(s).`);
}
