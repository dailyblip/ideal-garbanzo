...
const SNAPSHOT_PATH = 'data/h5-data-centers-jobs.json';
...
function runSelfTest() {
  ...
  if (fresh.expired) throw new Error('H5 fallback expired before 96 hours');
...
if (!decision.expired) {
  ...
  if (hasPublishedRoles) console.log(`H5 verification is ${age} hours old and inside the ${MAX_FALLBACK_AGE_HOURS}-hour maximum.`);
  else console.log('No H5 Data Centers roles are currently published; stale-fallback enforcement is not active.');
  process.exit(0);
}
...
status[SOURCE_KEY] = {
  ...
  fallbackFreshness: {
    ...
    policy: 'H5 Data Centers employer-direct roles may remain published for at most 96 hours after the last successful official ADP verification.'
  }
};
...
console.warn(`Expired H5 Data Centers snapshot after ${roundedAgeHours ?? 'unknown'} hours without official-source verification; removed ${removedPublic} public role(s) and ${removedSnapshot} snapshot role(s).`);
