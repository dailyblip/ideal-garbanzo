...
function validateFallback(payload) {
  ...
  if (!Number.isFinite(verifiedAt) || !Number.isFinite(expiresAt) || expiresAt <= verifiedAt) {
    violations.push('verifiedAt/expiresAt timestamps are invalid');
  }
  ...
  for (const job of payload.jobs) {
    ...
    if (!id) violations.push('fallback role is missing an id');
    else if (ids.has(id)) violations.push(`duplicate fallback id ${id}`);
    else ids.add(id);
    if (!url) violations.push(`${id || '(missing id)'} is missing sourceUrl`);
    else if (urls.has(url)) violations.push(`duplicate fallback URL ${url}`);
    else urls.add(url);
  }
...
const publicUrls = new Set(publicCoreSite.map(job => clean(job?.sourceUrl)).filter(Boolean));

if (!sourceHealthy && fallbackActive && Array.isArray(fallback?.jobs)) {
  const missing = fallback.jobs.filter(job => !publicIds.has(clean(job?.id)) && !publicUrls.has(clean(job?.sourceUrl)));
  if (missing.length) {
    violations.push(`active fallback lost ${missing.length}/${fallback.jobs.length} verified role(s): ${missing.slice(0, 5).map(job => clean(job?.id) || clean(job?.sourceUrl)).join(', ')}`);
  }
...
if (sourceHealthy) {
  console.log(`CoreSite fallback guard passed: direct source is healthy; ${publicCoreSite.length} public CoreSite role(s).`);
} else if (fallbackActive) {
  console.log(`CoreSite fallback guard passed: ${fallback.jobs.length}/${fallback.jobs.length} verified role(s) retained through ${fallback.expiresAt}.`);
} else {
  console.log(`CoreSite fallback guard passed: fallback is inactive/expired and no stale verified fallback roles remain.`);
}
